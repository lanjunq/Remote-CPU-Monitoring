...
extern int test_read_interface(char* file_path);
  ...
  while (1) { // keep looping and accept additional incoming connections
    ...
    if (fd != -1) {
      ...
      // this is the message that we'll send back
			...
			// Send interface.html
      int LIMIT = 10000;
      char * response = (char * ) malloc(LIMIT * sizeof(char));
			char * html_content = (char * ) malloc(LIMIT * sizeof(char));
			
			FILE* fp = fopen("interface.html", "r"); // Note: I changed the string to char* here, check if not work
			int content_length = fread(html_content, sizeof(char), LIMIT-1, fp);
